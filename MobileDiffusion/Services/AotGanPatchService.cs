        ...
        private SKBitmap RunAotGanModel(SKBitmap resizedImage, SKBitmap resizedMask)
        {
            ...
            for (int y = 0; y < ModelInputSize; y++)
            {
                for (int x = 0; x < ModelInputSize; x++)
                {
                    ...
                    bool isMask = maskPixel.Alpha > 10 && (maskPixel.Red > 10 || maskPixel.Green > 10 || maskPixel.Blue > 10);
                    var maskVal = isMask ? 1.0f : 0.0f;
                    maskArray[index] = maskVal;

                    // 2. Process Image
                    // Normalize to [-1, 1]
                    float rNorm = (pixel.Red / 255.0f - 0.5f) / 0.5f;
                    float gNorm = (pixel.Green / 255.0f - 0.5f) / 0.5f;
                    float bNorm = (pixel.Blue / 255.0f - 0.5f) / 0.5f;
                    
                    // Mask the input image (make masked area -1 i.e., 0 in pixel space)
                    if (maskVal > 0.5f)
                    {
            ...
            for (int y = 0; y < ModelInputSize; y++)
            {
                ...
                {
                    // layout is 1, 3, H, W
                    float rRaw = outputTensor[0, 0, y, x];
                    float gRaw = outputTensor[0, 1, y, x];
                    float bRaw = outputTensor[0, 2, y, x];
                    
                    // Denormalize from [-1, 1] to [0, 255]
                    int r = (int)((rRaw * 0.5f + 0.5f) * 255.0f);
                    int g = (int)((gRaw * 0.5f + 0.5f) * 255.0f);
                    int b = (int)((bRaw * 0.5f + 0.5f) * 255.0f);
        ...
        private SKRectI GetBoundingBox(SKBitmap mask)
        {
            ...
            for (int y = 0; y < mask.Height; y++)
            {
                for (int x = 0; x < mask.Width; x++)
                {
                    ...
                    // FIX: Ensure we don't treat opaque black background as mask
                    if (color.Alpha > 10 && (color.Red > 10 || color.Green > 10 || color.Blue > 10)) 
                    {
                        if (x < minX) minX = x;
                        if (x > maxX) maxX = x;
                        if (y < minY) minY = y;
