...
### CanvasPage
- `Views/CanvasPage.xaml.cs`: root partial for bindable properties, collection subscriptions, surface invalidation, compiled binding setup, page lifecycle wiring, and layout wiring
...
### CanvasPageViewModel
- `ViewModels/Pages/CanvasPage/CanvasPageViewModel.cs`: root partial for injected services, observable properties, tool setup, and shared canvas state
- `ViewModels/Pages/CanvasPage/CanvasPageViewModel.Workflows.cs`: save, send, crop, flatten, patch, and workflow payload preparation orchestration
- `ViewModels/Pages/CanvasPage/CanvasPageViewModel.Navigation.cs`: incoming query handling plus applying or stitching returned canvas results
- `ViewModels/Pages/CanvasPage/CanvasPageViewModel.TextAndHistory.cs`: undo/history flow, text add/edit/delete/duplicate actions, history popup coordination, and the editing-state helpers those flows depend on
- `ViewModels/Pages/CanvasPage/CanvasPageViewModel.SourceImage.cs`: source-image changes, palette extraction, persisted overlay restore, autosave cleanup, media replacement, and bitmap decoding
- `ViewModels/Pages/CanvasPage/CanvasPageViewModel.Segmentation.cs`: segmentation model readiness, interactive mask commands, and segmentation state resets
...
The broadest remaining canvas partial is `CanvasPageViewModel.Workflows.cs`, but an additional viewmodel split is not currently justified. After consolidating the editing/history helpers, source-image lifecycle handling, segmentation handling, and canvas-action bitmap generation, the remaining breadth is mostly orchestration over the same observable canvas session. Revisit another split only if workflows or navigation paths grow a separate lifecycle that no longer shares most of the same state.
